...
instance Functor (State s) where
  (<$>) ::
    ...
    -> State s a
    -> State s b
  f <$> State g = State (\s -> let (a, t) = g s in (f a, t))
...
instance Apply (State s) where
  (<*>) ::
    ...
    -> State s a
    -> State s b 
  State f <*> State g = State (\s -> let (func, t) = f s
                                         (x, t')   = g t
                                     in (func x, t'))
...
findM p (x :. xs) = (\found -> if found then pure (Full x) else findM p xs) =<< p x
...
firstRepeat ::
  ...
  List a
  -> Optional a
firstRepeat x = eval (findM (State . lift2 (lift2 (,)) S.member S.insert) x) S.empty 
...
distinct ::
  ...
  List a
  -> List a
distinct x = eval (filtering (State . lift2 (lift2 (,)) S.member S.insert) x) S.empty
